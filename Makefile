...
typecheck: ## Run type checking with mypy
	uv run mypy youtube_transcript_tool

test: ## Run tests
	uv run pytest tests/

security-bandit: ## Run bandit security linter
	uv run bandit -r youtube_transcript_tool -c pyproject.toml
...
run: ## Run youtube-transcript-tool (usage: make run ARGS="...")
...
install-global: build ## Install globally with uv tool (rebuilds first to avoid cache issues)
	-uv tool uninstall youtube-transcript-tool 2>/dev/null
	uv tool install . --reinstall

uninstall-global: ## Uninstall global installation
	uv tool uninstall youtube-transcript-tool
