# youtube-transcript-tool - Project Specification
...
3. **Usage Pattern** - User-friendly command
   ...
   youtube-transcript-tool completion bash
   youtube-transcript-tool completion zsh
...
5. **Installation Methods**
   - **Temporary**: `eval "$(youtube-transcript-tool completion bash)"`
...
1. Create new command module in `youtube_transcript_tool/`
2. Import and add to CLI group:
   ```python
   from youtube_transcript_tool.new_command import new_command
...
```bash
cd /path/to/youtube-transcript-tool
mise use -g python@3.14
uv sync
uv tool install .
```

After installation, `youtube-transcript-tool` command is available globally.

### Local development

```bash
uv sync
uv run youtube-transcript-tool [args]
```
...
2. **Configure Trusted Publisher** at https://pypi.org/manage/account/publishing/
   ...
   - **PyPI Project Name**: `youtube-transcript-tool`
   - **Owner**: `dnvriend`
   - **Repository name**: `youtube-transcript-tool`
...
```bash
pip install youtube-transcript-tool
```
